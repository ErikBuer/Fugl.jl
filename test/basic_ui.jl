...
function main()
    ...
    text_state = Ref(EditorState("Enter text here..."))
    ...
    function MyApp()
        Row([
            ...
            IntrinsicColumn([
                    ...
                    Container(TextBox(text_state[];
                        on_change=(text) -> text_state[] = EditorState(text_state[], text),
                        on_focus_change=(focused) -> text_state[] = EditorState(text_state[]; is_focused=focused))
                    )
                ],
                padding=0
            )
