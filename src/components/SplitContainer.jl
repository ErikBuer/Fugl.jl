...
function interpret_view(container::HorizontalSplitContainerView, x_offset::Float32, y_offset::Float32, width::Float32, height::Float32, projection_matrix::Mat4{Float32}, cursor_position::Point2f, window_size::Size)
    # Calculate split position in pixels
    split_pixel = container.state.split_position <= 1.0f0 ?
                  container.state.split_position * width :
                  min(container.state.split_position, width - container.style.handle_thickness)
    ...
    # Render children
    if left_width > 0
        interpret_view(container.left, x_offset, y_offset, left_width, height, projection_matrix, cursor_position, window_size)
    end

    if right_width > 0
        interpret_view(container.right, x_offset + right_x, y_offset, right_width, height, projection_matrix, cursor_position, window_size)
    end
...
function interpret_view(container::VerticalSplitContainerView, x_offset::Float32, y_offset::Float32, width::Float32, height::Float32, projection_matrix::Mat4{Float32}, cursor_position::Point2f, window_size::Size)
    # Calculate split position in pixels
    split_pixel = container.state.split_position <= 1.0f0 ?
                  container.state.split_position * height :
                  min(container.state.split_position, height - container.style.handle_thickness)
    ...
    # Render children
    if top_height > 0
        interpret_view(container.top, x_offset, y_offset, width, top_height, projection_matrix, cursor_position, window_size)
    end

    if bottom_height > 0
        interpret_view(container.bottom, x_offset, y_offset + bottom_y, width, bottom_height, projection_matrix, cursor_position, window_size)
    end
