...
function IntrinsicColumn(children::Vector{<:AbstractView}; spacing=10.0, reduce_spacing_on_overflow=true, on_click::Function=() -> nothing)
    return IntrinsicColumnView(children, spacing, reduce_spacing_on_overflow, on_click)
end

@inline function IntrinsicColumn(children::AbstractView...; spacing=10.0, reduce_spacing_on_overflow=true, on_click::Function=() -> nothing)
    return IntrinsicColumnView(collect(AbstractView, children), spacing, reduce_spacing_on_overflow, on_click)
end
...
function apply_layout(view::IntrinsicColumnView, x::Float32, y::Float32, width::Float32, height::Float32)
    ...
    for child in view.children
        if preferred_height(child)
            push!(intrinsic_heights, measure_height(child, width))
    ...
    remaining_height = max(0f0, height - total_spacing - total_intrinsic_height)
    ...
    for (i, child) in enumerate(view.children)
        ...
        push!(child_layouts, (x, child_y, width, child_height))
...
function interpret_view(view::IntrinsicColumnView, x::Float32, y::Float32, width::Float32, height::Float32, projection_matrix::Mat4{Float32}, cursor_position::Point2f, window_size::Size)
...
function measure_width(view::IntrinsicColumnView, available_height::Float32)::Float32
    ...
    # For a column, width is the maximum width of any child
...
function measure_height(view::IntrinsicColumnView, available_width::Float32)::Float32
    ...
    # Total height is sum of child heights plus spacing
