"""
NumberField - A simplified input field for numeric values.

This composite component provides:
- Single-line numeric input only
- Configurable numeric type (Int, Float64, etc.)
- Simple character filtering using Julia's built-in functions
...
function NumberField(
    ...
    on_change::Function = (new_state) -> nothing,
    on_focus_change::Function = (focused) -> nothing
)::NumberFieldView{T} where T<:Real
    return NumberFieldView{T}(state, style, on_change, on_focus_change)
end
...
function interpret_view(view::NumberFieldView{T}, x::Float32, y::Float32, width::Float32, height::Float32, projection_matrix::Mat4{Float32}) where T<:Real
    ...
    text_box = TextBox(
        ...
        on_change = (new_editor_state) -> begin
            # Parse and validate the new text
            new_text = new_editor_state.text
            ...
            view.on_change(new_number_state)
        end,
        on_focus_change = (focused) -> begin
            new_editor_state = EditorState(view.state.editor_state; is_focused=focused)
            new_number_state = NumberFieldState{T}(
                new_editor_state,
                view.state.options,
            ...
            view.on_focus_change(new_number_state)
        end
    )
