...
function draw_axes_with_labels(
    ...
    y_ticks::Vector{Float32},
    transform_func::Function,
    screen_bounds_points::Rectangle,
    color::Vec4{Float32},
    width::Float32,
    ...
    if show_x_tick_labels
        for x_tick in x_ticks
            if x_tick >= plot_bounds.x && x_tick <= plot_bounds.x + plot_bounds.width
                ...
                interpret_view(text_component, label_x, label_y, text_width, text_height, projection_matrix_points, Point2f(0.0f0, 0.0f0))
            end
        end
    end
    ...
    if show_x_label && !isempty(x_label)
        ...
        interpret_view(x_label_text, x_label_x, x_label_y, x_label_width, x_label_height, projection_matrix_points, Point2f(0.0f0, 0.0f0))
    end

    if show_y_label && !isempty(y_label)
        # Create Text component for y-axis label (rotated -90 degrees using Rotate component)
        y_label_style = TextStyle(size_points=label_size_points + 4, color=label_color)  # Style without rotation
        ...
        interpret_view(y_label_text, y_label_x, y_label_y, y_label_width, y_label_height, projection_matrix_points, Point2f(0.0f0, 0.0f0))
    end
end
