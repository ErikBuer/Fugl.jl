...
A draggable overlay component that displays content on top of background content with a semi-transparent overlay.

It defaults to the center of the parent area, but can be placed anywhere, by using the `offset_x` `offset_y` state varaibles.
...
screenshot(MyApp, "modal.png", 800, 600);
nothing #hide
```
